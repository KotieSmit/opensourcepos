...
class Item extends CI_Model
{
	...
	*/
	function get_bom_info($item_id)
	{
		...
		$this->db->join('items', "item_bom.item_id = items.item_id");
		$this->db->where('item_bom.item_id', $item_id);
		return $this->db->get();
...
//		return $this->db->query($sql, array($item_id));
    ...
    */
	...
	function update_all_bom_cost(){
		...
		while ($old_cost != $new_cost){
			...
			$sql = "UPDATE ospos_items as i INNER JOIN view_item_bom_cost as vibc ON i.item_id = vibc.item_id SET i.cost_price = vibc. bom_line_cost where i.cost_from_bom = 1;";
			$this->db->query($sql);
	...
	*/
	...
	public function get_item_search_suggestions($search, $limit=25, $search_custom=0, $is_deleted=0)
	{
		...
		$this->db->select('item_id, name, cost_price');
		$this->db->from('items');
		$this->db->where('deleted', $is_deleted);
		$this->db->like('name', $search);
		$this->db->order_by('name', 'asc');
		...
		$this->db->select('item_id, item_number, cost_price');
		$this->db->from('items');
		$this->db->where('deleted', $is_deleted);
		$this->db->like('item_number', $search);
		$this->db->order_by('item_number', 'asc');
		...
		foreach($by_item_number->result() as $row)
		{
			$suggestions[] = $row->item_id.'|'.$row->item_number .'|'. $row->cost_price;
		}

		//Search by description
		$this->db->select('item_id, name, description, cost_price');
		$this->db->from('items');
		$this->db->where('deleted', $is_deleted);
		$this->db->like('description', $search);
		$this->db->order_by('description', 'asc');
		...
		{
			$entry = $row->item_id.'|'.$row->name .'|'. $row->cost_price;
